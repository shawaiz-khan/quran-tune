...
export default function Navbar() {
    ...
    return (
        <Disclosure as="nav" className="py-6 px-5 bg-white shadow-lg shadow-slate-100 nav-container">
            {({ open }) => (
                <>
                        ...
                        <div className="relative flex items-center justify-between">
                            ...
                            <div className="flex-1 flex items-center justify-between sm:items-stretch">
                                <div className="flex-shrink-0 flex items-center">
                                    <Link to="/" className="text-xl hide-tap-color font-bold flex items-center space-x-1">
                                        <span className="text-black">Quran<span className="text-green-500 font-bold">Tune</span></span>
                                    </Link>
                                </div>
                                        ...
                                        {navigation.map((item) => (
                                            <Link
                                                ...
                                                className={classNames(
                                                    ...
                                                    'px-3 py-2 rounded-md text-base font-bold hide-tap-color'
                                                )}
                                            >
                                                {item.name}
                                            </Link>
                            ...
                            {navigation.map((item) => (
                                <Disclosure.Button as={Link} key={item.name}
                                    ...
                                    className={classNames(
                                        ...
                                        'block px-3 py-2 rounded-md text-base font-bold hide-tap-color'
                                    )}>
                                    {item.name}
                                </Disclosure.Button>
                            ))}
